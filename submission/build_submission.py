...
SELF_DIR = Path(__file__).resolve().parent             # repo/submission/
ROOT     = SELF_DIR.parent                              # repo root (train.py 등)
SUBMISSION_YAML = SELF_DIR / "submission.yaml"

# Kaggle 런타임 번들 — main.py import chain 따라 결정.
# (path, source_dir) 쌍 — SELF_DIR = submission/ 내부, ROOT = repo root.
BUNDLE_FILES: list[tuple[str, Path]] = [
    ("main.py",                 SELF_DIR),
    ("submission_features.py",  SELF_DIR),
    ("submission_actor.py",     SELF_DIR),
    ("train.py",                ROOT),
    ("model.py",                ROOT),
    ("env_wrapper.py",          ROOT),
    ("prediction.py",           ROOT),
...
def extract_pt_dims(ckpt: dict) -> dict:
    ...
    return {
        ...
        "PLANET_DIM":      int(planet_feat) + 1,  # obs 저장 dim (= FEAT + is_valid sentinel)
...
def extract_code_dims() -> dict:
    ...
    # submission_* 는 SELF_DIR, env_wrapper 는 ROOT 에 있음.
...
def bundle(pt_path: Path, out_path: Path) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # 1) 필수 파일/디렉토리 복사 (각 항목의 source 디렉토리에서)
        for fname, src_dir in BUNDLE_FILES:
            src = src_dir / fname
            if not src.exists():
                fail(f"번들 대상 파일 없음: {src}")
            shutil.copy2(src, staging / fname)

        for dname, src_dir in BUNDLE_DIRS:
            src = src_dir / dname
            if not src.exists():
